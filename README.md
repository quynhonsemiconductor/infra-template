...
| :-------- | :------- | :---- | :--------- |
| **AWS full-stack** | Postgres + background workers (SQS/SNS), private VPC, container services. | [`live/`](./live/) | Per-product **RDS + Fargate** (api/worker/migrator). Shares the **VPC / NAT / ALB / WAF** from `infra`'s `runtime-dev` / `runtime-prod` (Option A) via remote state — it does **not** create its own network. |
...
> first — `infra/live/runtime-dev` (key `platform/runtime-dev/...`) and
...
| :---- | :--- | :--------- | :------------------------- |
| **Singletons** | `infra` | The one-per-account resources: GitHub OIDC provider, KMS CMK, Tofu state bucket + lock table, artifacts bucket. | **No** — physically one per AWS account. Every product references them. |
| **Building blocks** | `tf-modules` | Versioned, reusable Terraform modules (network, rds, ecs-cluster, cdn, …). A **menu**, not a mandate. | **Yes** — pick what fits. An EKS product ignores `ecs-cluster`. A Lambda product uses almost none. |
...
`tf-modules` (versioned, tagged) and have both reference it. Used by one
...
live/
  ...
  develop/   Develop environment stack. Provisions this product's own RDS +
             ...
             from infra runtime-dev via remote state (does NOT create its
...
1. **`infra` bootstrap AND the shared runtime layer applied** — the
...
2. **GitHub secrets** on the new repo: `AWS_ACCOUNT_ID`, plus the Cloudflare
   ...
   zone ID and IP ranges are read from `infra` bootstrap via `_shared`
...
- OIDC-only auth (no static AWS keys) via `ci/actions/setup-tofu-aws@v1`.
